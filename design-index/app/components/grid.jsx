...
export default function Grid({items, category}) {
 let filtered = [];
  ...
  return (
    <>
    {filtered.length > 0 ? (
      <div className="h-[80%] overflow-auto grid grid-cols-1 sm:grid-cols-4 gap-[10px] sm:gap-[28px] xl:gap-[20px] mt-[10px] w-full pb-[100px] sm:pb-[50px] pt-[5px] px-[5px] hide-scrollbar items-start">
          {filtered.map((item) =>
              item ? (
              <>
              ...
              <img
                ...
                <Link
                      key={item.tool_name}
                      to={`/tool/${item.tool_name.toLowerCase()}`}
                      className="sm:hidden bg-white rounded-[15px] lg:rounded-[20px] xl:rounded-[24px] justify-start items-start border-[1px] border-[#e1e1e1] flex flex-row hover:shadow-sm hover:translate-y-[-2px] transition-all duration-200 ease-in-out cursor-pointer"
                  >
